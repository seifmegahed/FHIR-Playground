...
export async function getPatientsByPractitioner(
  id: string,
  page: number,
  count: number
) {
  ...
  return await fetch(
    `https://spark.incendi.no/fhir/Patient?_count=${count}&_offset=${_offset}&_sort=name&_sortDirection=asc&general-practitioner=${id}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/fhir+json",
